...
const MovieRow = ({ title, items, onCardClick, category, genreId }) => {
  ...
  // We don't want to return null while loading, so we check for items *or* loading state
  if (!rowItems?.length && !loading) {
    return null;
  }
    ...
    <section className="space-y-3">
      ...
      <div className="relative">
        ...
        <div className="no-scrollbar flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth pb-2">
          {loading ? (
            <div className="w-full flex justify-center items-center h-[290px]">
              <p>Loading...</p>
            </div>
          ) : (
            rowItems.map((item) => (
